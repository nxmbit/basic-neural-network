...
void feed_forward(neural_network_s *network) {
    ...
    for (int i = 0; i < network->layers_count - 1; i++) {
        ...
        matrix_copy(network->layers[i]->weighed_sums, temp_neurons);
        for (int j = 0; j < network->layers[i + 1]->layer_size; j++) {
            if (i == network->layers_count - 2) {
                network->layers[i + 1]->neurons->tab[j][0] = softmax(network->layers[i]->weighed_sums->tab[j][0], network->layers[i]->weighed_sums->tab, network->layers[i + 1]->layer_size);
            } else {
                network->layers[i + 1]->neurons->tab[j][0] = tanh(network->layers[i]->weighed_sums->tab[j][0]);
            }
        }
    ...
    for (int i = network->layers_count - 1; i > 0; i--) {
        ...
        if (i == network->layers_count - 1) {
            ...
            for (int j = 0; j < network->layers[i]->layer_size; j++) {
                ...
                subtraction_result->tab[j][0] = d_softmax(network->layers[i - 1]->weighed_sums->tab[j][0], network->layers[i - 1]->weighed_sums->tab, network->layers[i - 1]->weighed_sums->rows) * error;
            }
            ...
            for (int j = 0; j < network->layers[i]->layer_size; j++) {
                ...
                network->layers[i - 1]->neurons_delta->tab[j][0] = d_tanh(network->layers[i - 1]->weighed_sums->tab[j][0]) * error;
...
void stochastic_network_train(neural_network_s *network, dataset_s *dataset, int epochs, double learning_rate, int batch_size) {
    ...
    for (int epoch = 0; epoch < epochs; epoch++) {
        ...
        epoch_cost /= (double) batch_size;
        epoch_success_rate /= (double) batch_size;
        printf("Epoch: %d out of %d, cost: %lf, success rate: %lf\n", epoch + 1, epochs, epoch_cost, epoch_success_rate);
